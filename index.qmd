...
# highest CBP for cities over 1 million
#cpal_1mil %>%
#  filter(YEAR == max(YEAR)) %>%
#  arrange(desc(cbp_3y)) %>%
#  mutate(RANK = 1+lag(n = 1))
```
...
plotcbpLine <- cpal_dallas %>%
  ...
  geom_text(aes(label = paste0(round(cbp_3y*100, digits = 2), "%")),
...
cpal_plot(zeroGrob(),
          cpal_title(string = "Child Poverty Rate in the City of Dallas (3-year average)",
                     width = 75),
          plotcbpLine, 
...
# Between 2021 and 2022 the Child Poverty Rate in all large cities continues to decrease relative to prior years.
...
plot1MilBar <- cpal_1mil1YrCh %>%
  ...
  geom_text(aes(label = paste0(round(cbpChange*100, digits = 0), "%")),
            x = -0.007,
            color = "white",
            fontface = "bold") +
...
cpal_plot(zeroGrob(),
          cpal_title(string = paste0("Percent Change in Child Poverty in Cities Over 1,000,000 Population Between 2021 and ", max(cpal_1mil$YEAR)),
                     width = 75),
          plot1MilBar, 
          cpal_source(text = "U.S. Census Bureau. American Community Survey Table S1702, 1-year estimates. (2017-2022)"),
          cpal_note(text = "2020 ACS Estimates are derived from 5-Year ACS due to pandemic collection issues."),
...
# Since 2017 the City of Dallas has had the fourth largest drop in Child Poverty compared to its peer metropolitan cities.
...
```{r, Poverty by Race/Ethnicity in Cities Over 1,000,000 Population}
...
cpal_plot(zeroGrob(),
          cpal_title(string = "Poverty Rate by Race/Ethnicity in Cities Over 1,000,000 Population",
                     width = 75),
          plotRace1Mil, 
          cpal_source(text = "U.S. Census Bureau. American Community Survey Table S1702, 1-year estimates. (2017-2022)"),
          cpal_note(text = "2020 ACS Estimates are derived from 5-Year ACS due to pandemic collection issues."),
...
# Since 2017 the City of Dallas has had the second largest drop in Child Poverty compared to its peer Texas cities.
