...
cpal_1milCh <- cpal_1mil %>%
  ...
  select(GEOID, NAME, CODE, YEAR, highlight, bp_u18_3y, bp_u18_1y) %>%
  pivot_wider(names_from = YEAR, values_from = c(bp_u18_3y, bp_u18_1y)) %>%
  rename(OLD3y = 5,
         NEW3y = 6,
         OLD1y = 7,
         NEW1y = 8,
         ) %>%
  mutate(cbpChange1y = (NEW1y-OLD1y)/OLD1y,
         cbpChange3y = (NEW3y-OLD3y)/OLD3y)
...
plotcbpLine <- cpal_dallas %>%
  ...
  mutate(name = ifelse(name == "cbp_3y", "CPAL 3-Year Average", "ACS 1-Year Survey")) %>%
...
cpal_dallas %>%
  ...
  mutate(name = ifelse(name == "childpov3y", "CPAL 3-Year Average", "ACS 1-Year Survey")) %>%
...
plot1MilBar <- cpal_1mil1YrCh %>%
  ...
  mutate(name = ifelse(name == "cbpChange3y", "CPAL 3-Year Average", "ACS 1-Year Survey")) %>%
  mutate(CODE = factor(CODE, levels = c("DAL", "NYC", "LA", "CHI", "HOU", "PHO", "SAN", "PHI", "SD", "SJ"))) %>%
  ggplot(aes(x=value, y=CODE, fill = highlight)) +
  geom_bar(stat = "identity", width = 0.8) +
  geom_text(aes(label = paste0(round(value*100, digits = 0), "%"),
                x = 0.05),
                color = palette_cpal_main[3],
                fontface = "bold") +
  facet_wrap(~name) +
...
### Child Poverty Metrics Cities Over 1,000,000 Table
```{r, Child Poverty Table Cities Over 1,000,000 Million}
cpal_1mil %>%
  ungroup(.) %>%
  ...
  rename('City' = CODE,
         'CPAL 3-Year Average' = childpov3y,
